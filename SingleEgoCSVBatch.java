 ...
 * @author sid
 */
public class SingleEgoCSVBatch {
    
     public void script(String node_id,int hop_number,String output_folder_path) {

         System.err.println(node_id+"in script");
    ...
    try {
        File file_node = new File(getClass().getResource("/org/gephi/toolkit/demos/one_hop_node_folders/"+node_id+"_node.csv").toURI());
        ...
        File file_edge = new File(getClass().getResource("/org/gephi/toolkit/demos/one_hop_edge_folders/"+node_id+"_edge.csv").toURI());
    ...
    importController.process(container2, new AppendProcessor(), workspace);
        ...
        String query_node_id= node_id;
        Node node_ = graph.getNode(query_node_id);
        System.out.println("QUERY NODE"+query_node_id);
       ...
       // System.out.println(query_node);
        ...
        egoFilter.setPattern(node_id); //Regex accepted
        egoFilter.setDepth(hop_number);
...
//
        ...
        try {
            String path = output_folder_path+"/"+node_.getId().toString()+"_graph.png";
...
public void batcher(String location_of_nodes)
{
 

    
    
    File folder = new File(location_of_nodes);
    File[] listOfFiles = folder.listFiles();

    for (File file : listOfFiles) {
        if (file.isFile()) {
            
           String node_string =  file.getName().split("\\.")[0];
           String node_id    = node_string.split("_")[0];
           System.out.println(node_id+"is node id");
           String output_folder = "/home/sid/Desktop/CODE/Internship-Indian-Institute-of-Science/COVID IISc/gephi_graphs";
           
               
            //creating object 
            SingleEgoCSVBatch s = new SingleEgoCSVBatch();
            s.script(node_id, 1,output_folder);
